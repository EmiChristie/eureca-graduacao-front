        ...
        <Stat.Root>
          <Flex h={"full"} flexDir={"column"}>
            <Text textAlign={"justify"} fontWeight={"normal"} color={`${EURECA_COLORS.CINZA}/80`}>Atualmente, sua taxa de sucesso acadêmico é igual a {fda.valor_do_aluno == 1? "1" : fda.valor_do_aluno.toFixed(2)}.</Text>
            {
                ...
                :
                    <Text mt={2} textAlign={"justify"} fontWeight={"normal"} color={`${EURECA_COLORS.CINZA}/80`}>A probabilidade acumulada para a sua taxa de sucesso é de {fda.percentil.toFixed(2)}. Isso significa que sua taxa de sucesso é mais alta que {fda.percentil < 0.5 ? "apenas" : ""} {(parseFloat(fda.percentil.toFixed(2))*100).toFixed(2)}% das taxas de sucesso dos alunos ativos de seu curso. Consequentemente, você está entre os {(parseFloat((1-fda.percentil).toFixed(2))*100).toFixed(2)}% alunos ativos do seu curso com {fda.percentil < 0.5 ? "menores":"maiores"} taxas de sucesso.</Text>
