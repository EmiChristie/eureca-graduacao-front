      ...
      <Card.Body>
        <Stat.Root>
          <Flex h={"full"} flexDir={"column"}>
            ...
            <Text mt={2} textAlign={"justify"} fontWeight={"normal"} color={`${EURECA_COLORS.CINZA}/80`}>A probabilidade acumulada para o seu CRA é de {fda.percentil.toFixed(2)}. Isso significa que seu CRA é mais alto que {fda.percentil < 0.5 ? "apenas" : ""} {(parseFloat(fda.percentil.toFixed(2))*100).toFixed(2)}% dos CRAs dos alunos ativos de seu curso. Consequentemente, você está entre os {(parseFloat((1-fda.percentil).toFixed(2))*100).toFixed(2)}% alunos ativos do seu curso com {fda.percentil < 0.5 ? "menores":"maiores"} CRAs.</Text>
          </Flex>
        </Stat.Root>
      </Card.Body>
    </Card.Root>
