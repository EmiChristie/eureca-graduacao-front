                ...
                <Flex mt={2} h={"full"} alignItems={"center"}>
                    {
                        ...
                        <Text color={`${EURECA_COLORS.CINZA}/80`} fontWeight={"medium"}>
                            No currículo de {codigo_do_curriculo} do curso de {formatarNome(curso.descricao)}, a disciplina de {formatarNome(nome)} é ofertada como <Span fontWeight={"normal"} color={EURECA_COLORS.AZUL_MEDIO} bg={`${EURECA_COLORS.AZUL_CLARO}/20`} p={1} rounded={"xs"}>componente {mapearTipo(tipo)}</Span>. Idealmente, a disciplina é cursada no <Span fontWeight={"normal"} color={EURECA_COLORS.AZUL_MEDIO} bg={`${EURECA_COLORS.AZUL_CLARO}/20`} p={1} rounded={"xs"}>{semestre_ideal}º período</Span> com uma carga horária de <Span fontWeight={"normal"} color={EURECA_COLORS.AZUL_MEDIO} bg={`${EURECA_COLORS.AZUL_CLARO}/20`} p={1} rounded={"xs"}>{horas_totais} horas</Span> (equivalentes a {quantidade_de_creditos} créditos).
                        </Text>
                        :
                        <Text color={`${EURECA_COLORS.CINZA}/80`} fontWeight={"medium"}>
                            No currículo de {codigo_do_curriculo} do curso de {formatarNome(curso.descricao)}, a disciplina de {formatarNome(nome)} é ofertada como <Span fontWeight={"normal"} color={EURECA_COLORS.AZUL_MEDIO} bg={`${EURECA_COLORS.AZUL_CLARO}/20`} p={1} rounded={"xs"}>componente {mapearTipo(tipo)}</Span>. A disciplina não está associada a um período ideal dentro do fluxograma e possui carga horária de <Span fontWeight={"normal"} color={EURECA_COLORS.AZUL_MEDIO} bg={`${EURECA_COLORS.AZUL_CLARO}/20`} p={1} rounded={"xs"}>{horas_totais} horas</Span> (equivalentes a {quantidade_de_creditos} créditos).
                        </Text>
                    }
                </Flex>
                </Stat.Root>
